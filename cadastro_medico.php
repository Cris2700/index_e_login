    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        ...
        try {
            $stmt = $pdo_login->prepare(
              "INSERT INTO dentistas (nome_completo, nome_mae, cpf, telefone, email, cep, rua, numero, bairro, cidade, estado, senha, cro, data_nascimento) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
            );
            $stmt->execute([$nome, $nome_mae, $cpf, $telefone, $email, $cep, $rua, $numero, $bairro, $cidade, $estado, $senha_hash, $cro, $data_nascimento]);
